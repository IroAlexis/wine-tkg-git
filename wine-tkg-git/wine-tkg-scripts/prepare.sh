...
_prepare() {
	...
	if [ "$_use_fastsync" = "true" ]; then
	  if [ "$_use_staging" != "true" ]; then
	    _use_esync="false"
	    _use_fsync="false"
	    if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 5df0f5f6fb90e1326b71cd1094a7b710b94916d4 HEAD ); then
...
EOM
	...
	if [ "$_proton_fs_hack" = "true" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_use_staging" = "true" ]; then
	    if git merge-base --is-ancestor 62cb6ace2cfe46358e6526868145a5bd8a7f990b HEAD; then
	      ...
	      if git merge-base --is-ancestor a27d5bae114eef5352b699ee38975bc8793b4dcb HEAD; then
	        _lastcommit="62cb6ac"
	...
	if [ "$_EXTERNAL_INSTALL" = "proton" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_sdl_joy_support" = "true" ]; then
	    if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 27f40156baa7f1e09c6e420f6c278606557a505a HEAD ) && [ "$_use_staging" = "true" ]; then
